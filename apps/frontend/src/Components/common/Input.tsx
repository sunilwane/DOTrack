    ...
    return (
        <div className={`space-y-1.5 ${containerClassName}`}>
            ...
            <input
                ...
                className={`w-full h-11 bg-slate-50 dark:bg-slate-600/50 border border-slate-100 dark:border-slate-600 rounded-lg px-4 text-sm hover:border-primary/60 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all placeholder:text-slate-500 text-slate-900 dark:text-white ${error ? 'border-red-500' : ''} ${className}`}
                {...props}
            />
            {error && (
